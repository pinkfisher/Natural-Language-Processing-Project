...
class FeatureBasedSequenceScorer(object):
    ...
    Attributes:
        ...
        emission_log_probs: [num_tags, num_words] matrix containing emission log probabilities (tag, word)
    """
    def __init__(self, tag_indexer: Indexer, feature_indexer: Indexer, weights):
        self.tag_indexer = tag_indexer
...
class CrfNerModel(object):
    def __init__(self, tag_indexer, feature_indexer, feature_weights):
        ...
        self.scorer = FeatureBasedSequenceScorer(self.tag_indexer, self.feature_indexer, self.feature_weights)

    def _forward_backward(self, features):
        # generate a viterbi score matrix whose size is number of states * steps
        num_tags = len(self.tag_indexer)

        # initialize start probability
        self.alpha = np.zeros((len(features), num_tags))
        self.alpha[0, :] = [self.scorer.score_potential(features[0][tag_idx]) for tag_idx in range(num_tags)]
        ...
        for step in range(1, len(features)):
            for tag in range(num_tags):
                ...
                self.alpha[step, tag] += self.scorer.score_potential(features[step][tag])
    ...
    def fit(self, features, gold_labels):
        num_tags = len(self.tag_indexer)
        self._forward_backward(features)
        for i in range(len(features)):
            ...
            # substract marginals
            for tag in range(num_tags):
                # marginals = (self.alpha[i, tag] + self.beta[i, tag]) / self.Z[i]
    ...
    def viterbi(self, features):
        ...
        # initialize start probability
        score[0, :] = [self.scorer.score_potential(features[0][tag_idx]) for tag_idx in range(num_tags)]
        ...
        for step in range(1, len(features)):
            for tag in range(num_tags):
                ...
                score[step, tag] = np.max(potential) + self.scorer.score_potential(features[step][tag])
    ...
    def decode(self, sentence_tokens):
        ...
        pred_tags = self.viterbi(features)
        return LabeledSentence(sentence_tokens, chunks_from_bio_tag_seq(pred_tags))
...
def train_crf_model(sentences):
    ...
    crf = CrfNerModel(tag_indexer, feature_indexer, weights)
    optimizer = UnregularizedAdagradTrainer(weights, eta=0.1)
    ...
    for epoch_index in range(num_epoch):
        ...
        while sentence_idx < end_idx:
                crf.zero_gradients()
                crf.fit(feature_cache[sentence_idx], true_labels[sentence_idx])
